...
RUN wget https://mirrors.tuna.tsinghua.edu.cn/apache/zookeeper/zookeeper-3.4.10/zookeeper-3.4.10.tar.gz && \
	...
	wget https://mirrors.tuna.tsinghua.edu.cn/apache/hbase/${HIVE_VERSION}/hbase-${HIVE_VERSION}-bin.tar.gz && \
	tar -xzvf hbase-${HIVE_VERSION}-bin.tar.gz && \
    mv hbase-${HIVE_VERSION} /usr/local/hbase && \
    rm hbase-${HIVE_VERSION}-bin.tar.gz && \
